...
describe('QueueJournal', () => {
  ...
  it('claims independent messages in one guarded append', async () => {
    const memory = new MemoryClient();
    const journal = new QueueJournal(memory as unknown as UrsulaClient);
    await journal.enqueue(queueName, { runId: 'batch-one' });
    await journal.enqueue(queueName, { runId: 'batch-two' });
    await journal.enqueue(queueName, { runId: 'batch-three' });
    const appendsBeforeClaim = memory.appendedBatchSizes.length;
    ...
    expect(memory.appendedBatchSizes.slice(appendsBeforeClaim)).toEqual([3]);
  });

  it('claims at most one message per concurrency key in a batch', async () => {
    const memory = new MemoryClient();
    const journal = new QueueJournal(memory as unknown as UrsulaClient);
    await journal.enqueue(queueName, { runId: 'same-run' });
    await journal.enqueue(queueName, { runId: 'same-run' });
    await journal.enqueue(queueName, { runId: 'other-run' });
    ...
    expect(
      leases
        .map(({ message }) => {
          if (!('runId' in message.message)) return '';
          return message.message.runId;
        })
        .sort()
    ).toEqual(['other-run', 'same-run']);
  });

  it('does not duplicate leases when dispatchers claim one batch concurrently', async () => {
    const memory = new MemoryClient();
    ...
    for (let index = 0; index < 20; index += 1) {
      await first.enqueue(queueName, { runId: `contended-${index}` });
    }
    memory.yieldBeforeAppend = true;

    const claims = await Promise.all([
      first.claimMany(queueName, new Date(), 10_000, 20),
      second.claimMany(queueName, new Date(), 10_000, 20),
    ]);
    const leases = claims.flat();

    expect(leases).toHaveLength(20);
    expect(new Set(leases.map(({ leaseId }) => leaseId))).toHaveLength(20);
    expect(new Set(leases.map(({ message }) => message.messageId))).toHaveLength(
      20
    );
    expect(memory.appendedBatchSizes.filter((size) => size === 20)).toHaveLength(
      1
    );
  });
